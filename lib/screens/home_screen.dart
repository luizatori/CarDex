...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void handleCardClick(CarItem car) {
    setState(() {
      selectedCar = car;
    });

    final carsProvider = Provider.of<CarsProvider>(context, listen: false);
    final totalColecao = carsProvider.filledCount;

    // LOGICA DE TRANSICAO:
    if (!car.isEmpty) {
      // 1. SE O CARD ESTA POPULADO - abre a visualizacao 3D/parallax
      Navigator.of(context).push(
        PageRouteBuilder(
          ...
          pageBuilder: (context, _, __) => CarExpandedView(
            ...
            heroTag: 'car_hero_${car.imageUrl ?? car.name}',
          ),
        ),
      );
    } else {
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [
                            (isDark ? const Color(0xFFE7E5E5) : Colors.black).withOpacity(0.0),
                            (isDark ? const Color(0xFFE7E5E5) : Colors.black).withOpacity(0.8), 
                            (isDark ? const Color(0xFFE7E5E5) : Colors.black).withOpacity(0.0),
                          ],
